...
# Unserialize the documents variable
#file = open('pickles/data/documents.pickle', 'rb')
...
# Unserialize the word features
#file = open('pickles/data/wordfeatures.pickle', 'rb')
...
# Unserialize the feature sets
#file = open('pickles/data/featuresets.pickle', 'rb')
...
pickle.dump(NBclassifier, classifier_file)
classifier_file.close()

# Unpickle the trained NLTK NB Classifier
#classifier_file = open('pickles/classifiers/nltknaivebayes.pickle', 'rb')
#NBclassifier = pickle.load(classifier_file)
#classifier_file.close()

# Test the NLTK NB Classifier
print('NLTK Naive Bayes accuracy:', nltk.classify.accuracy(NBclassifier, testing_set) * 100)
utils.print_precision_recall(NBclassifier, testing_set)
...
voted_classifier = VoteClassifier(NBclassifier, MNB_classifier, BNB_classifier, 
...
# Classify the first test sample as an example
...
def sentiment(text):
    features = utils.get_features(text, word_features)
...
prediction, confidence = sentiment(text1)
print('Classification:', prediction, 'Confidence:', confidence)
